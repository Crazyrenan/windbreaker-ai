...
import { 
  Navigation, Loader2, PlaneTakeoff, Search, LogOut, User 
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      alert("Error: Pastikan backend FastAPI sudah berjalan dan sesi Anda masih valid!");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen text-slate-100 flex items-center justify-center p-4 md:p-8 font-sans overflow-x-hidden bg-[#0f172a] selection:bg-blue-500 selection:text-white">
      <div className="max-w-6xl w-full grid grid-cols-12 gap-6">
        ...
        <header ref={headerRef} className="col-span-12 mb-4 flex justify-between items-center border-b border-slate-800 pb-6">
          ...
          <div className="flex items-center gap-6">
            <div className="hidden md:block text-right">
              <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em]">Logged in as</p>
            ...
            <button 
              ...
              className="bg-red-500/10 hover:bg-red-500/20 text-red-500 p-3 rounded-xl border border-red-500/20 transition-all flex items-center justify-center group"
              title="Logout"
            >
        ...
        <div ref={formRef} className="col-span-12 lg:col-span-7 bg-slate-900/40 border border-slate-800 p-6 md:p-10 rounded-4xl backdrop-blur-md shadow-2xl">
          ...
          <button 
            ...
            disabled={loading}
            className="w-full mt-10 bg-blue-600 hover:bg-blue-500 active:scale-[0.98] text-white font-black py-5 rounded-2xl transition-all shadow-xl shadow-blue-900/20 uppercase tracking-[0.2em] text-xs flex items-center justify-center gap-3"
          >
            {loading ? <Loader2 className="animate-spin" size={18} /> : 'Analyze Flight Risk'}
          </button>
        </div>

        <div className="col-span-12 lg:col-span-5 h-full min-h-125">
          {result ? (
            <div ref={cardRef} className="h-full bg-linear-to-b from-slate-900 to-black border border-slate-800 p-10 rounded-5xl flex flex-col items-center justify-center text-center relative overflow-hidden shadow-2xl">
              ...
              <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-6">Delay Probability</h3>
              <h3 className="text-7xl font-black mb-6 tracking-tighter italic">{result.risk_score}<span className="text-3xl not-italic opacity-50">%</span></h3>
              <div className={`py-4 px-8 rounded-2xl border font-black uppercase tracking-widest text-sm ${result.risk_score > 40 ? 'bg-red-500/20 text-red-400 border-red-500/30' : 'bg-green-500/20 text-green-400 border-green-500/30'}`}>
